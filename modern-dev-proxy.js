/**
 * Replit Development Proxy - ESM Version
 * 
...
// Configuration
const DEV_SERVER_PORT = process.env.PORT || 5000;
...
// Function to update .env file with the necessary domain
function setupEnvFile() {
  ...
  try {
    ...
    // Get the Replit domain from env or use a fallback
    const replitDomain = process.env.REPLIT_DOMAIN || 'c4c63b04-8137-449e-93ce-043996d346d0-00-2ytirv4mg9c5l.kirk.replit.dev';
    
    // Ensure VITE_ALLOWED_HOSTS includes our domains
    if (!envContent.includes('VITE_ALLOWED_HOSTS=')) {
      envContent += `\nVITE_ALLOWED_HOSTS=${replitDomain},localhost,127.0.0.1,*.replit.dev,*.replit.co`;
      fs.writeFileSync(envFile, envContent);
      console.log('Updated .env file with required host configuration');
    }
  } catch (err) {
...
server.listen(PROXY_PORT, '0.0.0.0', () => {
  console.log(`\x1b[32m[DEV-PROXY]\x1b[0m Proxy server running on port ${PROXY_PORT}`);
  console.log(`\x1b[32m[DEV-PROXY]\x1b[0m Proxying to: http://localhost:${DEV_SERVER_PORT}`);
  ...
  if (process.env.REPL_ID) {
    console.log(`\x1b[32m[DEV-PROXY]\x1b[0m The application is now accessible from problematic hosts`);
  }
});

// Handle graceful shutdown
process.on('SIGINT', () => {
  console.log('\x1b[33m[DEV-PROXY]\x1b[0m Shutting down the proxy server...');
  server.close(() => {
    console.log('\x1b[33m[DEV-PROXY]\x1b[0m Server shut down successfully');
    process.exit(0);
  });
});