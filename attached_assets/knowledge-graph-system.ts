...
export class KnowledgeGraphService {
  ...
  public async createFromDocument(documentId: number, content?: string): Promise<KnowledgeGraph> {
    try {
      ...
      if (!content) {
        ...
        content = documentContent.extractedText;
      }
      ...
      const { nodes, edges } = await this.generateBasicGraph(content, document.title, documentId);
   ...
   * @param graphId - Knowledge graph ID
   * @returns Promise<any> - Graph statistics
   */
  public async getGraphStatistics(graphId: string): Promise<any> {
    try {
      const graph = this.graphs.get(graphId);
      if (!graph) {
        throw new Error(`Knowledge graph not found: ${graphId}`);
      ...
      // Count node types
      const nodeTypeDistribution: {[key: string]: number} = {};
      for (const node of graph.nodes) {
        nodeTypeDistribution[node.type] = (nodeTypeDistribution[node.type] || 0) + 1;
      }
      
      // Count edge types
      const edgeTypeDistribution: {[key: string]: number} = {};
      ...
      const avgNodeDegree = graph.nodes.length > 0 ? totalDegree / graph.nodes.length : 0;
      
      // Calculate simple density score (ratio of actual edges to possible edges)
      const maxPossibleEdges = graph.nodes.length * (graph.nodes.length - 1) / 2;
      const densityScore = maxPossibleEdges > 0 ? graph.edges.length / maxPossibleEdges : 0;
      ...
      return {
        ...
        avgNodeDegree,
        densityScore,
        // For a real implementation, we would add more complex metrics
        // like betweenness centrality, clustering coefficient, etc.
      };
    } catch (error) {
      this.logger.error(`Error getting graph statistics: ${error instanceof Error ? error.message : String(error)}`);
      throw error;
    }
  }
}
